...
import { Package, Tag, ArrowRight, Plus, AlertTriangle, Settings, PieChart, ShoppingBag, PlusCircle } from "lucide-react";
import Image from "next/image";
import { useProductsStore } from "@/lib/store/productsStore";

interface CatalogoDashboardProps {
  basePath: string; // e.g. "/recepcion/catalogo" or "/gerencia/catalogo"
}

export default function CatalogoDashboard({ basePath }: CatalogoDashboardProps) {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <Link href={`${basePath}/productos`} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col xl:flex-row items-center justify-between gap-3 hover:border-sky-500/30 hover:bg-sky-500/5 transition-all group relative overflow-hidden shadow-sm hover:shadow-sky-900/10 cursor-pointer">
             <div className="flex items-center gap-3 w-full">
               <div className="w-10 h-10 rounded-xl bg-sky-500/10 flex items-center justify-center text-sky-500 group-hover:scale-110 transition-transform shrink-0">
                 <Package size={18} />
               </div>
               <div className="text-center xl:text-left flex-1">
                 <p className="text-sm font-black text-white group-hover:text-sky-400 transition-colors">Productos</p>
                 <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest hidden xl:block">Gestión de stock y precios</p>
               </div>
             </div>
          </Link>
          
          <Link href={`${basePath}/categorias`} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col xl:flex-row items-center justify-between gap-3 hover:border-orange-500/30 hover:bg-orange-500/5 transition-all group relative overflow-hidden shadow-sm hover:shadow-orange-900/10 cursor-pointer">
             <div className="flex items-center gap-3 w-full">
               <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500 group-hover:scale-110 transition-transform shrink-0">
                 <PieChart size={18} />
               </div>
               <div className="text-center xl:text-left flex-1">
                 <p className="text-sm font-black text-white group-hover:text-orange-400 transition-colors">Categorías</p>
                 <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest hidden xl:block">Estructura del menú</p>
               </div>
             </div>
          </Link>

          <Link href={`${basePath}/envios`} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col xl:flex-row items-center justify-between gap-3 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all group relative overflow-hidden shadow-sm hover:shadow-emerald-900/10 cursor-pointer">
             <div className="flex items-center gap-3 w-full">
               <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 group-hover:scale-110 transition-transform shrink-0">
                 <Settings size={18} />
               </div>
               <div className="text-center xl:text-left flex-1">
                 <p className="text-sm font-black text-white group-hover:text-emerald-400 transition-colors">Zonas de Envío</p>
                 <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest hidden xl:block">Barrios y costos</p>
               </div>
             </div>
          </Link>

          <Link href={`${basePath}/extras`} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col xl:flex-row items-center justify-between gap-3 hover:border-pink-500/30 hover:bg-pink-500/5 transition-all group relative overflow-hidden shadow-sm hover:shadow-pink-900/10 cursor-pointer">
             <div className="flex items-center gap-3 w-full">
               <div className="w-10 h-10 rounded-xl bg-pink-500/10 flex items-center justify-center text-pink-500 group-hover:scale-110 transition-transform shrink-0">
                 <PlusCircle size={18} />
               </div>
               <div className="text-center xl:text-left flex-1">
                 <p className="text-sm font-black text-white group-hover:text-pink-400 transition-colors">Extras</p>
                 <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest hidden xl:block">Opciones adicionales</p>
               </div>
             </div>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <Link href={`${basePath}/productos`} className="text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-sky-400 flex items-center gap-1 transition">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <Link href={`${basePath}/productos`} className="text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-orange-400 flex items-center gap-1 transition">
